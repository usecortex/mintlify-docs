...
- Raw text and markdown content
- User/assistant conversation pairs
- **Inference mode** (`infer=true`): Extracts additional insights and implicit information from content
- **Direct mode** (`infer=false`): Content is indexed as-is for faster processing
...
### 4. Memory Management
- **Remove outdated memories** with the [Delete Memory](/api-reference/endpoint/delete-memory) endpoint
- **Maintain data quality** through regular memory audits
- **Track processing status** using the [Verify Processing](/api-reference/endpoint/verify-processing) endpoint
...
1. **Add your first memory** using the [Add Memory](/api-reference/endpoint/add-memories) endpoint
...
4. **Delete memories** using the [Delete Memory](/api-reference/endpoint/delete-memory) endpoint
