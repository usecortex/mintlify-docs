...
openapi: "POST /ingestion/upload_document"
---
...
For documents, the identifier is `id`. When that identifier already exists, `upsert` (boolean, default `true`) controls the behavior:
...
<Info>
  **Need Help?** If a document fails to process or you're experiencing issues, contact our support team with the `id` for assistance.
</Info>