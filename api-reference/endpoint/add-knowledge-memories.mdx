...
<Info>
**Supported formats** include PDF, DOCX, TXT, and other common document types. See the request schema and API response for the full list and any format-specific options.
</Info>

<Warning>
**Unsupported File Formats**: If you attempt to upload a file format that is not supported, you will receive an error response with status code `400` and the message: `"Unsupported file format: [filename]."` Ensure your files are in a supported format (e.g. PDF, DOCX, TXT) before uploading.
</Warning>
