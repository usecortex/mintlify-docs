...
The `upsert` parameter (boolean, default `true`) controls what happens when you upload content whose `file_id` already exists in your tenant:

| Value | Behavior |
|-------|----------|
| `true` | Replace the existing content with the new one |
| `false` | Fail the request; do not overwrite |

When `upsert` is `true` (the default), uploading again with the same `file_id` **completely replaces** the existing content: the new content is processed (validation, chunking, embedding generation, indexing), previous search results and embeddings for that `file_id` are replaced, and uploading the same content with the same `file_id` multiple times is safe and idempotent.

<Warning>
**Important**: When overwriting existing content, all previous chunks, embeddings, and search indexes associated with that `file_id` are permanently removed and replaced. This cannot be undone.
</Warning>
