...
The `upsert` parameter (boolean, default `true`) controls what happens when you upload a document whose `file_id` already exists in your tenant:

| Value | Behavior |
|-------|----------|
| `true` | Replace the existing document with the new one |
| `false` | Fail the request; do not overwrite |

When `upsert` is `true` (the default), uploading again with the same `file_id` **completely replaces** the existing document: the new file is processed (content extraction, chunking, embedding generation, indexing), previous search results and embeddings for that `file_id` are replaced, and uploading the same document with the same `file_id` multiple times is safe and idempotent.

<Warning>
**Important**: When overwriting an existing document, all previous chunks, embeddings, and search indexes associated with that `file_id` are permanently removed and replaced. This cannot be undone.
</Warning>
