...
### When a `file_id` already exists: the `upsert` parameter (optional)

The `upsert` parameter (boolean, default `true`) controls what happens when you upload an app source whose `file_id` already exists in your tenant:

| Value | Behavior |
|-------|----------|
| `true` | Replace the existing app source with the new one |
| `false` | Fail the request; do not overwrite |

When `upsert` is `true` (the default), uploading again with the same `file_id` **completely replaces** the existing app source: the new source is processed through its specialized pipeline, previous search results and embeddings for that `file_id` are replaced, and uploading the same app sources with the same `file_id`s multiple times is safe and idempotent.

<Warning>
**Important**: When overwriting existing app sources, all previous chunks, embeddings, and search indexes associated with those `file_id`s are permanently removed and replaced. This cannot be undone.
</Warning>

**Example success response when overwriting by `file_id`:**
