...
Controls whether Cortex extracts insights, likes, dislikes, preferences, and outcomes from the content:
...
Processing is asynchronous. The response includes a `results` array with an `id` and `status` for each item. Use these to track progress:


```bash
# Example: check status via ingestion verify-processing (when using file_id from ingestion flows)
curl -X POST \
  'https://api.usecortex.ai/ingestion/verify-processing?file_id=<id>&tenant_id=<tenant_id>' \
  -H 'Authorization: Bearer <token>'
```
...
<Note>
**Batch Uploads**: 
You can include multiple items in a single request for efficient bulk ingestion. Each item can have different `infer` settings.
