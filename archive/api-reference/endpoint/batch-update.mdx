---
title: 'Batch Update'
description: 'API reference for Batch Update.'
...
Update multiple sources in bulk, creating new entries for any `id` that doesn't exist (upsert operation).
> Provide one `id` per file. The number of `id`s must match the number of files.
